import Dashboard_Filter from "@/components/Filters";
import Popup_Filter from "@/components/ModalFilter";
  ...
  useEffect(() => {
    const newFilteredCoins = allCoins?.filter((coins) => {
      ...
      if (
        ...
        !coins.name.toLowerCase().includes(filterOptions.keyword.toLowerCase())
      ) {
        return false;
      }
          ...
          <div className="flex m-auto overflow-y-hidden gap-4 lg:gap-8 w-full h-[calc(88vh)]">
            ...
            <div className="flex w-full md:w-3/4 mx-auto justify-between container">
              <div className="p-4 overflow-y-auto w-full h-[calc(80vh)]">
                <h2 className="text-lg font-medium mb-4">Coins</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                  {filteredCoins?.status?.error_code === 429 ? (
                    <p>Limit exceeded</p>
                  ) : (
                        ...
                        <div className="flex flex-row justify-between items-start mt-4">
                          <div className="px-2">
                            <p className="text-sm text-gray-800 font-bold">
                              {coins.name.length > 30
                                ? coins.name.slice(0, 30) + "..."
                                : coins.name}
                            </p>
                            <p className="text-sm text-gray-800 pt-1.5">
                              ₹
                              {coins.market_cap.toString().length > 12
                                ? coins.market_cap.toString().slice(0, 12) +
                                  "..."
                                : coins.market_cap}
                            </p>
                          </div>
